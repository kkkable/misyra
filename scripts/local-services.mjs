...
function runDocker(args, { capture = false } = {}) {
  ...
  if (result.error?.code === 'ENOENT') {
    fail('Docker CLI is unavailable. Install or start Docker, then retry the local-service command.');
  }
...
async function checkHealth() {
  ...
  const portResult = runDocker(['compose', 'port', 'azurite', '10000'], { capture: true });
  const published = portResult.stdout.trim().split(/\r?\n/)[0] ?? '';
  const portMatch = /:(\d+)$/.exec(published);

  if (!portMatch) {
    fail(`Azurite blob endpoint is unavailable; Docker Compose reported port mapping: ${published || 'none'}`);
  }

  let response;
  try {
    response = await fetch(`http://127.0.0.1:${portMatch[1]}/devstoreaccount1?comp=list`, {
      signal: AbortSignal.timeout(5_000),
    });
  } catch (error) {
    fail(`Azurite blob endpoint is unavailable: ${error instanceof Error ? error.message : String(error)}`);
  }
...
async function main() {
  switch (command) {
    ...
    case 'reset':
      if (!yes) {
        fail('Reset removes only the named local PostgreSQL and Azurite volumes. Re-run with --yes to confirm intentional local data reset.');
      }
