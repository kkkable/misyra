/**
 * Build script for @misyra/worker.
 *
...
if (compile.status !== 0) {
  process.stderr.write(`@misyra/worker ${noEmit ? "typecheck" : "build"} failed\n`);
  process.exit(compile.status ?? 1);
}
process.stdout.write(`@misyra/worker ${noEmit ? "typecheck" : "build"} ok\n`);
