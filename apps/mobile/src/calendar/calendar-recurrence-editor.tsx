...
function initialPreset(recurrence: MissionRecurrence | null): Preset {
  ...
  switch (recurrence.pattern.type) {
    case 'daily': return recurrence.pattern.interval === 1 ? 'daily' : 'custom';
    case 'weekly': return recurrence.pattern.interval === 1 ? 'weekly' : 'custom';
    case 'monthly-date':
    case 'monthly-ordinal': return recurrence.pattern.interval === 1 ? 'monthly' : 'custom';
    case 'yearly-date':
    case 'yearly-ordinal': return recurrence.pattern.interval === 1 ? 'yearly' : 'custom';
  }
}
...
function initialCustomFrequency(recurrence: MissionRecurrence | null): CustomFrequency {
  ...
  switch (recurrence.pattern.type) {
    ...
    case 'yearly-date':
    case 'yearly-ordinal': return 'yearly';
  }
}
...
function initialOrdinal(recurrence: MissionRecurrence | null, fallback: Ordinal): Ordinal {
  if (recurrence?.pattern.type === 'monthly-ordinal' || recurrence?.pattern.type === 'yearly-ordinal') {
...
export function CalendarRecurrenceEditor({
  ...
  const weekdayLabels = [
    catalog['calendar.recurrence.weekday.sun'], catalog['calendar.recurrence.weekday.mon'],
    catalog['calendar.recurrence.weekday.tue'], catalog['calendar.recurrence.weekday.wed'],
    catalog['calendar.recurrence.weekday.thu'], catalog['calendar.recurrence.weekday.fri'],
    catalog['calendar.recurrence.weekday.sat'],
  ] as const;

  const [preset, setPreset] = useState<Preset>(() => initialPreset(initialRecurrence));
  const [customFrequency, setCustomFrequency] = useState<CustomFrequency>(() => initialCustomFrequency(initialRecurrence));
  const [interval, setInterval] = useState(() => initialInterval(initialRecurrence));
  const [weekdays, setWeekdays] = useState<number[]>(() => initialWeekdays(initialRecurrence, anchorWeekday));
  const [monthlyMode, setMonthlyMode] = useState<MonthlyMode>(() => initialMonthlyMode(initialRecurrence));
  const [yearlyMode, setYearlyMode] = useState<YearlyMode>(() => initialYearlyMode(initialRecurrence));
  const [ordinal, setOrdinal] = useState<Ordinal>(() => initialOrdinal(initialRecurrence, anchorOrdinal(anchorDay)));
  const [ordinalWeekday, setOrdinalWeekday] = useState(() => initialOrdinalWeekday(initialRecurrence, anchorWeekday));
  ...
  const choosePreset = (value: Preset) => {
    ...
    if (value === 'monthly') { setMonthlyMode('date'); setDay(String(anchorDay)); }
    if (value === 'yearly') { setYearlyMode('date'); setMonth(String(anchorMonth)); setDay(String(anchorDay)); }
    if (value === 'custom') { setCustomFrequency('weekly'); setWeekdays([anchorWeekday]); }
  };
      ...
      {orderedWeekdays.map((weekday) => {
        ...
        return (
          <Pressable
            ...
            accessibilityState={singleSelection ? { checked: ordinalWeekday === weekday } : { checked: weekdays.includes(weekday) }}
            key={weekday}
            onPress={() => { if (singleSelection) setOrdinalWeekday(weekday); else toggleWeekday(weekday); }}
            style={optionStyle}
            testID={`recurrence-weekday-${String(weekday)}`}
          >
            <Text allowFontScaling style={textStyle}>{label}</Text>
          </Pressable>
        );
      })}
    </View>
  ...
  const renderOrdinalButtons = () => (
    <View style={styles.compactRow}>
      {([
        ...
        <Pressable accessibilityLabel={label} accessibilityRole="radio" accessibilityState={{ checked: ordinal === value }} key={id} onPress={() => { setOrdinal(value); }} style={optionStyle} testID={`recurrence-ordinal-${id}`}>
          <Text allowFontScaling style={textStyle}>{label}</Text>
        </Pressable>
      ))}
    </View>
  ...
  const buildPattern = (): RecurrencePattern | null => {
    ...
    if (effectiveFrequency === 'monthly') {
      if (monthlyMode === 'ordinal') return { type: 'monthly-ordinal', interval: parsedInterval, ordinal, weekday: ordinalWeekday };
      const dayOfMonth = boundedInteger(day, 1, 31);
      return dayOfMonth === null ? null : { type: 'monthly-date', interval: parsedInterval, dayOfMonth };
    }
    const parsedMonth = boundedInteger(month, 1, 12);
    if (parsedMonth === null) return null;
    if (yearlyMode === 'ordinal') return { type: 'yearly-ordinal', interval: parsedInterval, month: parsedMonth, ordinal, weekday: ordinalWeekday };
    const parsedDay = boundedInteger(day, 1, 31);
    return parsedDay === null ? null : { type: 'yearly-date', interval: parsedInterval, month: parsedMonth, day: parsedDay };
  };

  const finish = () => {
    if (preset === 'none') { onDone(null); return; }
  ...
  return (
    <View accessibilityRole="summary" style={[styles.editor, { borderColor: colors.border }]} testID="calendar-recurrence-editor">
      <Text allowFontScaling style={[styles.heading, { color: colors.textPrimary }]}>{catalog['calendar.recurrence.title']}</Text>
      <View style={styles.wrapRow}>
        {([
          ...
          <Pressable accessibilityLabel={label} accessibilityRole="radio" accessibilityState={{ checked: preset === value }} key={value} onPress={() => { choosePreset(value); }} style={optionStyle} testID={testID}>
            <Text allowFontScaling style={textStyle}>{label}</Text>
          </Pressable>
        ))}
      </View>
      ...
      {preset === 'custom' ? (
        <View style={styles.wrapRow}>
          {([
            ...
            <Pressable accessibilityLabel={label} accessibilityRole="radio" accessibilityState={{ checked: customFrequency === value }} key={value} onPress={() => { setCustomFrequency(value); setInterval('1'); if (value === 'weekly') setWeekdays([anchorWeekday]); }} style={optionStyle} testID={testID}>
              <Text allowFontScaling style={textStyle}>{label}</Text>
            </Pressable>
          ))}
        </View>
      ...
      {preset === 'none' ? null : <TextInput accessibilityLabel={catalog['calendar.recurrence.interval']} keyboardType="number-pad" onChangeText={setInterval} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-interval" value={interval} />}
      {effectiveFrequency === 'weekly' ? renderWeekdayButtons(false) : null}

      {effectiveFrequency === 'monthly' ? (
        <>
          <View style={styles.wrapRow}>
            <Pressable accessibilityLabel={catalog['calendar.recurrence.sameDate']} accessibilityRole="radio" accessibilityState={{ checked: monthlyMode === 'date' }} onPress={() => { setMonthlyMode('date'); }} style={optionStyle} testID="recurrence-monthly-date"><Text allowFontScaling style={textStyle}>{catalog['calendar.recurrence.sameDate']}</Text></Pressable>
            <Pressable accessibilityLabel={catalog['calendar.recurrence.ordinalWeekday']} accessibilityRole="radio" accessibilityState={{ checked: monthlyMode === 'ordinal' }} onPress={() => { setMonthlyMode('ordinal'); setOrdinal(anchorOrdinal(anchorDay)); setOrdinalWeekday(anchorWeekday); }} style={optionStyle} testID="recurrence-monthly-ordinal"><Text allowFontScaling style={textStyle}>{catalog['calendar.recurrence.ordinalWeekday']}</Text></Pressable>
          </View>
          {monthlyMode === 'date' ? <TextInput accessibilityLabel={catalog['calendar.recurrence.dayOfMonth']} keyboardType="number-pad" onChangeText={setDay} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-day" value={day} /> : <>{renderOrdinalButtons()}{renderWeekdayButtons(true)}</>}
        </>
      ) : null}
      ...
      {effectiveFrequency === 'yearly' ? (
        <>
          <View style={styles.wrapRow}>
            <Pressable accessibilityLabel={catalog['calendar.recurrence.sameDate']} accessibilityRole="radio" accessibilityState={{ checked: yearlyMode === 'date' }} onPress={() => { setYearlyMode('date'); }} style={optionStyle} testID="recurrence-yearly-date"><Text allowFontScaling style={textStyle}>{catalog['calendar.recurrence.sameDate']}</Text></Pressable>
            <Pressable accessibilityLabel={catalog['calendar.recurrence.ordinalWeekday']} accessibilityRole="radio" accessibilityState={{ checked: yearlyMode === 'ordinal' }} onPress={() => { setYearlyMode('ordinal'); setOrdinal(anchorOrdinal(anchorDay)); setOrdinalWeekday(anchorWeekday); }} style={optionStyle} testID="recurrence-yearly-ordinal"><Text allowFontScaling style={textStyle}>{catalog['calendar.recurrence.ordinalWeekday']}</Text></Pressable>
          </View>
          <TextInput accessibilityLabel={catalog['calendar.recurrence.month']} keyboardType="number-pad" onChangeText={setMonth} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-month" value={month} />
          {yearlyMode === 'date' ? <TextInput accessibilityLabel={catalog['calendar.recurrence.dayOfMonth']} keyboardType="number-pad" onChangeText={setDay} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-day" value={day} /> : <>{renderOrdinalButtons()}{renderWeekdayButtons(true)}</>}
        </>
      ) : null}

      <Text allowFontScaling style={[styles.sectionHeading, { color: colors.textPrimary }]}>{catalog['calendar.recurrence.ends']}</Text>
      <View style={styles.wrapRow}>
        {([
          ...
          <Pressable accessibilityLabel={label} accessibilityRole="radio" accessibilityState={{ checked: endMode === value }} key={value} onPress={() => { setEndMode(value); }} style={optionStyle} testID={testID}><Text allowFontScaling style={textStyle}>{label}</Text></Pressable>
        ))}
      </View>
      {endMode === 'date' ? <TextInput accessibilityLabel={catalog['calendar.recurrence.onDate']} onChangeText={setEndDate} placeholder={catalog['calendar.recurrence.localDateInputHint']} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-end-date-input" value={endDate} /> : null}
      {endMode === 'count' ? <TextInput accessibilityLabel={catalog['calendar.recurrence.afterCount']} keyboardType="number-pad" onChangeText={setEndCount} style={[styles.input, { borderColor: colors.border, color: colors.textPrimary }]} testID="recurrence-end-count-input" value={endCount} /> : null}
      <View style={styles.actions}>
        <Pressable accessibilityLabel={catalog['calendar.create.cancel']} accessibilityRole="button" onPress={onCancel} style={optionStyle}><Text allowFontScaling style={textStyle}>{catalog['calendar.create.cancel']}</Text></Pressable>
        <Pressable accessibilityLabel={catalog['calendar.recurrence.done']} accessibilityRole="button" onPress={finish} style={optionStyle} testID="recurrence-done"><Text allowFontScaling style={textStyle}>{catalog['calendar.recurrence.done']}</Text></Pressable>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  sectionHeading: { fontSize: typography.body.fontSize, fontWeight: typography.body.mediumFontWeight },
  wrapRow: { flexDirection: 'row', flexWrap: 'wrap', gap: space[2] },
  compactRow: { flexDirection: 'row', flexWrap: 'wrap', gap: space[1] },
  option: { alignItems: 'center', borderRadius: radius.md, borderWidth: 1, justifyContent: 'center', minHeight: layout.minimumTouchTarget, minWidth: layout.minimumTouchTarget, paddingHorizontal: space[3], paddingVertical: space[2] },
