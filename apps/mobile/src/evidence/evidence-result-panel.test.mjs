...
describe('MTS-084 retained evidence actions', () => {
  ...
  it('hides retained-media actions after app-controlled media is deleted', () => {
    ...
    expect(renderer.root.findAllByProps({ testID: 'evidence-result-save-to-photos' })).toHaveLength(
      0,
    );
    expect(renderer.root.findAllByProps({ testID: 'evidence-result-delete-media' })).toHaveLength(
      0,
    );
  });
});
