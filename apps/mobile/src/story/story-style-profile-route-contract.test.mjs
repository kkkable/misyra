...
const setupRoutePath = fileURLToPath(
  new URL('../../app/story-style-profile.tsx', import.meta.url),
);
const pickerPath = fileURLToPath(
  new URL('./story-style-profile-reference-picker.ts', import.meta.url),
);
const mediaApiPath = fileURLToPath(
  new URL('./story-style-profile-media-api.ts', import.meta.url),
);
...
describe('MTS-093 Story style-profile mobile integration', () => {
  ...
  it(
    'gates only a new first Story and leaves an existing draft independent from later profile changes',
    () => {
  ...
  it(
    'exposes Story style-profile management from Settings with rebuild and reset-default actions',
    () => {
