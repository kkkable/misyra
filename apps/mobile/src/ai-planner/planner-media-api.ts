import type { SystemPickedImage } from './ai-planner-input.js';

type PlannerMediaApiOptions = Readonly<{
  baseUrl: string;
  accessToken: string;
}>;
...
function payloadFromEnvelope(value: unknown): unknown {
  if (!isRecord(value) || value.ok !== true || !Object.hasOwn(value, 'payload')) {
    throw new Error('planner_media_request_failed');
  }
  return value.payload;
}

function imageContentType(image: SystemPickedImage): string {
  ...
  throw new Error('planner_image_content_type_unavailable');
}
...
function parseAuthorization(value: unknown, assetId: string): UploadAuthorization {
  if (!isRecord(value) || value.assetId !== assetId || typeof value.uploadPath !== 'string') {
    throw new Error('planner_media_authorization_invalid');
  }
  return { assetId, uploadPath: value.uploadPath };
}

export function createPlannerMediaApi({ baseUrl, accessToken }: PlannerMediaApiOptions) {
  const root = baseUrl.endsWith('/') ? baseUrl.slice(0, -1) : baseUrl;
  const authorization = `Bearer ${accessToken}`;

  return Object.freeze({
    async uploadOriginal(assetId: string, image: SystemPickedImage): Promise<void> {
      ...
      if (!response.ok) throw new Error('planner_media_authorization_failed');
      const upload = parseAuthorization(payloadFromEnvelope(responseBody), assetId);

      const localFile = await fetch(image.uri);
      if (!localFile.ok) throw new Error('planner_local_image_unavailable');
      ...
      if (!uploadResponse.ok) throw new Error('planner_media_upload_failed');
    },
  });
}
