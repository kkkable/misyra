...
describe('MTS-039 PostgreSQL device registration and account settings', () => {
  it('keeps one stable device id for the same account installation while refreshing metadata', async () => {
  ...
  it('shares Trust Mode and app language across devices without overwriting other account settings', async () => {
    const authStore = createPostgresAuthStore(pool);
    const store = createPostgresDeviceSettingsStore(pool);
    ...
    await expect(
      store.updateAccountSettings(account.id, { language: 'zh-HK', trustMode: true }),
