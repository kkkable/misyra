/**
 * Build script for @misyra/domain.
 *
...
if (compile.status !== 0) {
  process.stderr.write(`@misyra/domain ${noEmit ? "typecheck" : "build"} failed\n`);
  process.exit(compile.status ?? 1);
}
process.stdout.write(`@misyra/domain ${noEmit ? "typecheck" : "build"} ok\n`);
