...
async function loadMediaRetentionFunctions() {
  ...
  return {
    ...
    shouldGenerateMediaDeletionNotice: requireFunction(
      module,
      'shouldGenerateMediaDeletionNotice',
    ),
  };
}
...
describe('MTS-021 media-retention calculations contract', () => {
  it.each([
    ...
    'planner-working',
    'style-references',
  ])('assigns the approved 30-day deadline to %s', async (purpose) => {
  ...
  it('classifies feedback media outside product-media cleanup', async () => {
    ...
    expect(calculateMediaDeletionDeadline('2026-08-01T12:00:00Z', 'feedback-retained')).toBe(
      null,
    );
  });
