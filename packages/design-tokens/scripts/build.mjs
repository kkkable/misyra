/**
 * Build script for @misyra/design-tokens.
 *
...
if (compile.status !== 0) {
  process.stderr.write(`@misyra/design-tokens ${noEmit ? "typecheck" : "build"} failed\n`);
  process.exit(compile.status ?? 1);
}
process.stdout.write(`@misyra/design-tokens ${noEmit ? "typecheck" : "build"} ok\n`);
