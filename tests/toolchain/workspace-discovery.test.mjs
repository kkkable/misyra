...
test("declared workspace directories each contain a valid package.json", () => {
  ...
  for (const dir of dirs) {
    ...
    assert.match(
      manifest.name,
      /^@misyra\//,
      `workspace package must use the @misyra scope: ${dir}`,
    );
    assert.equal(
      typeof manifest.version,
      "string",
      `workspace package must declare a version: ${dir}`,
    );
    assert.equal(manifest.private, true, `workspace package must be private: ${dir}`);
    assert.equal(
      basename(dir),
      manifest.name.split("/")[1],
      `directory name must match package name: ${dir}`,
    );
  }
});
...
test("the toolchain fixture workspace is discoverable through pnpm", () => {
  ...
  assert.ok(
    dirs.includes("toolchain-fixture"),
    "expected packages/toolchain-fixture to be discoverable",
  );
});
...
test("exactly one lockfile exists: pnpm-lock.yaml at the repository root", () => {
  ...
  const competitors = walkFiles(repoRoot, (name) =>
    ["package-lock.json", "yarn.lock", "bun.lockb", "bun.lock", "npm-shrinkwrap.json"].includes(
      name,
    ),
  );
...
test("pnpm recursively discovers the same workspaces as pnpm-workspace.yaml", () => {
  ...
  const listed = /** @type {{ name: string }[]} */ (JSON.parse(raw));
  const actual = listed
      ...
      for (const [dep, range] of Object.entries(deps)) {
        if (dep.startsWith("@misyra/")) {
          ...
          assert.match(
            String(range),
            /^workspace:/,
            `internal dependency must use a workspace: range: ${dep}`,
          );
        }
      }
    }
