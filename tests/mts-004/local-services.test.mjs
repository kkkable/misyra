...
test('compose defines healthy local PostgreSQL 18 and Azurite services', () => {
  ...
  assert.equal(existsSync(serviceScriptPath), true, 'local service command script must exist');
  ...
  assert.match(compose, /image:\s*mcr\.microsoft\.com\/azure-storage\/azurite:[^\s]+/);
...
test('database connectivity integration test', { skip: !runIntegration }, () => {
  const started = runDocker(['compose', 'up', '-d', '--wait', 'postgres']);
  assert.equal(started.status, 0, `PostgreSQL did not become healthy:\n${started.stderr || started.stdout}`);
  ...
  assert.equal(probe.status, 0, `PostgreSQL connectivity probe failed:\n${probe.stderr || probe.stdout}`);
...
test('Azurite container test', { skip: !runIntegration }, async () => {
  ...
  assert.equal(started.status, 0, `Azurite did not become healthy:\n${started.stderr || started.stdout}`);

  const port = process.env.AZURITE_BLOB_PORT ?? '10000';
  const response = await fetch(`http://127.0.0.1:${port}/devstoreaccount1?comp=list`, {
    signal: AbortSignal.timeout(5_000),
  });

  assert.ok(response.status >= 200 && response.status < 500, `unexpected Azurite status ${response.status}`);
});
